...
addEventHandler("onClientResourceStart", resourceRoot, function()
	for i = 346, 358 do
		for _,v in pairs(emptyWeaponModels) do
			if (i == v) then
				local tex = engineLoadTXD("mods/alpha.txd", i);
				engineImportTXD(tex, i);
				local mod = engineLoadDFF("mods/alpha.dff", i);
				engineReplaceModel(mod, i);
			end
		end
	end
	-- standart txd skin textures
	local txd = engineLoadTXD("mods/73.txd", v);
	engineImportTXD(txd, 73);
	local dff = engineLoadDFF("mods/73.dff", v);
	engineReplaceModel(dff, 73);
	local txd = engineLoadTXD("mods/73_1.txd", v);
	engineImportTXD(txd, 71);
	local dff = engineLoadDFF("mods/73.dff", v);
	engineReplaceModel(dff, 71);
	local txd = engineLoadTXD("mods/73_2.txd", v);
...
bindKey("u", "down", "chatbox", "globalchat");
