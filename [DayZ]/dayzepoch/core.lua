...
addEventHandler("onPlayerChat", root, function(message, mType)
	...
	if (mType == 0) then
		if exports.asia_security:isAdvertisement(message) then
		...
		if isPlayerMuted(source) then
			outputChatBox(getLanguageTextServer("clientinfotext43",source), source, 136,8,8);
			return;
		end
		local x,y,z = getElementPosition(source);
		...
		if (getElementData(source,"antichat")) then
			outputChatBox(getLanguageTextServer("clientinfotext41",source), source, 136,8,8);
			return;
		else
...
addCommandHandler("give", function(source, _, target, amount)
	...
	local zKills = getPlayerMoney(source);
	if target and target ~= source then
		if tonumber(amount) >= 1 and zKills >= tonumber(amount) then
			takePlayerMoney (source, tonumber(amount))
			givePlayerMoney(target, amount)
			outputChatBox("#880808[DayZ PH]#D3D3D3 You sent "..tostring(amount).." (PHP) to "..getPlayerName(target):gsub("#%x%x%x%x%x%x", "").."!",source,211,211,211,true);
			outputChatBox("#880808[DayZ PH]#D3D3D3 You received "..tostring(amount).." (PHP) from "..getPlayerName(source):gsub("#%x%x%x%x%x%x", "").."!",target,211,211,211,true);
		else
			outputChatBox("#880808[DayZ PH]#D3D3D3 You don't have enough money (PHP)!",source,136,8,8,true);
		end
	else
		outputChatBox("Player not found!",source,136,8,8)
	end
end);
...
addCommandHandler("globalchat", function(player, _, ...)
	if (configVar.globalchat) then
		...
		if exports.asia_security:isAdvertisement(globalAccount) then
			outputChatBox("#880808[Warning!] #D3D3D3Promoting or engaging in advertising is strictly prohibited, and continued violation may lead to a ban.", source, 211, 211, 211,true)
			return;
		end
