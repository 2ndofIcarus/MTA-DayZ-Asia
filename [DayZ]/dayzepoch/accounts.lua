...
function spawnDayZPlayer(player)
	if player then
		...
		for _,v in ipairs(playerDataTable) do
			if (v[1] ~= "skin" and v[1] ~= "gpschannel") then
				setElementData(player, v[1], v[2]);
			end
		end
		...
		if not isGuestAccount(getPlayerAccount(player)) then
			if getElementData(player, "logedin") then
				if (getElementModel(player) == 0) then
					...
					spawnDayZPlayer(player);
					outputChatBox(getPlayerName(player).."'s account is buggy and has been reset.", getRootElement(), 136,8,8);
					return;
				end
				for _,v in ipairs(playerDataTable) do
...
addCommandHandler("kill", function(player)
	if (getPlayerSkin(player) == 290) then
		outputChatBox("#880808[DayZ PH]#D3D3D3 You cannot kill yourself while on duty!",player,0,0,0,true)
		cancelEvent()
	else
